...
class Purchaserequester(models.Model):
        ...
        for rec in self:
            if rec.state == 'acc':
                for compradores in rec.proyecto_compradores:
                    ...
                    _logger.info("====>", compradores.id)
                    _logger.info("====>", compradores.name)
    ...
    limit_date_order = fields.Datetime('Fecha de Límite de Solicitud', required=True, index=True, tracking=True, copy=False,
                             ...
                             help="Representa la fecha en la que se debe validar la cotización y convertirla en una orden de compra.", states={'acc': [('readonly', True)]})
